...
const ITEMS = [
  ...
  // ==================== WINDOWS FULL / BALANCED / EXTREME ====================
  {
    ...
    requiresAdmin: true,
    confirm: false,
    profiles: ['performance', 'gaming', 'advanced'],
    proOnly: true,
    icon: 'gauge',
  ...
  {
    id: 'win.inputlag.smooth',
    name: 'Reduzir input lag (agendador multimídia)',
    category: 'windows',
    description: 'Reduz a reserva de CPU feita pelo serviço multimedia (MMCSS) e desativa o limitador de tráfego de rede que causa micro-stutters.',
    benefit: 'Sistema e jogos respondem mais rápido ao mouse/teclado (menos micro-travadas).',
    risk: 'medium',
    requiresAdmin: true,
    confirm: false,
    profiles: ['performance', 'gaming'],
    proOnly: true,
    rebootRequired: true,
    apply: ps('Set-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile" -Name SystemResponsiveness -Value 10 -Type DWord; New-Item -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\NetworkThrottling" -Force | Out-Null; Set-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\NetworkThrottling" -Name NetworkThrottlingIndex -Value 0xFFFFFFFF -Type DWord'),
    undo: ps('Remove-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile" -Name SystemResponsiveness -ErrorAction SilentlyContinue; Remove-ItemProperty -Path "HKLM:\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile\\NetworkThrottling" -Name NetworkThrottlingIndex -ErrorAction SilentlyContinue'),
    registryKeys: ['HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Multimedia\\SystemProfile']
  },
  {
    id: 'win.foreground.boost',
    name: 'Prioridade da janela ativa (Windows)',
    category: 'windows',
    description: 'Configura o escalonador do Windows para dar mais tempo de CPU à janela em primeiro plano (programas respondem mais rápido).',
    benefit: 'Janela focado (jogo/app) recebe boost de CPU em relação aos processos em segundo plano.',
    risk: 'medium',
    requiresAdmin: true,
    confirm: false,
    profiles: ['performance', 'gaming'],
    proOnly: true,
    rebootRequired: true,
    apply: ps('Set-ItemProperty -Path "HKLM:\\SYSTEM\\CurrentControlSet\\Control\\PriorityControl" -Name Win32PrioritySeparation -Value 38 -Type DWord'),
  ...
  {
    id: 'win.timers.stable',
    name: 'Timers estáveis de alta resolução (bcdedit)',
    category: 'windows',
    description: 'Desativa o tick dinâmico do kernel e o relógio de plataforma, deixando os timers mais precisos (reduz latência de entrada em jogos).',
    benefit: 'Micro-stutters e delay de entrada reduzidos em jogos competitivos. Efeito varia por máquina.',
    risk: 'high',
    requiresAdmin: true,
    confirm: true,
    profiles: ['performance', 'gaming'],
    proOnly: true,
    rebootRequired: true,
    apply: ps('bcdedit /set disabledynamictick yes; bcdedit /set useplatformclock false'),
    undo: ps('bcdedit /deletevalue disabledynamictick; bcdedit /deletevalue useplatformclock'),
    registryKeys: []
  },
  {
    id: 'net.dns.flush',
