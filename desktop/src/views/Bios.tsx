  ...
  const openBiosGuide = async (id: string, item: BiosItem | null) => {
    try {
        ...
        body: (
          <div>
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              O SevenOptimizer não afirma que a opção foi aplicada até verificar o hardware de novo.
            </p>
          </div>
        ),
      });
      if (verify && api.biosScheduleVerify) {
        ...
        await refreshBios();
        toast('info', 'Operação pendente criada. Após reiniciar, o SevenOptimizer verifica o resultado.');
      }
    } catch (err) {
      ...
      body: (
        <div>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            O SevenOptimizer tentará reverter a alteração usando o snapshot salvo. Se não for possível, será indicado um rollback manual.
          </p>
        </div>
      ),
    });
  ...
  const handleReboot = async () => {
    ...
    const ok = await ask({
      ...
      body: (
        <div>
          <p className="m-0">
            Salve seu trabalho antes de continuar. Após a reinicialização, o SevenOptimizer verificará automaticamente as otimizações pendentes.
          </p>
          <p className="mt-3 flex items-start gap-2 text-xs text-amber-400">
            <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
      ...
      {pendingCount > 0 && (
        <div className="flex flex-wrap items-center gap-3 rounded-lg bg-amber-500/10 px-4 py-3 text-sm text-amber-400">
          ...
          <span className="flex-1">
            Há otimizações aguardando reinicialização. O SevenOptimizer confirmará o resultado na próxima inicialização.
          </span>
