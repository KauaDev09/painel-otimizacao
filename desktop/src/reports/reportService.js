...
function buildTxt(result) {
  ...
  const L = [];
  L.push('==============================================================');
  L.push('  SevenOptimizer — RELATÓRIO DE ANÁLISE DE BIOS');
  ...
  L.push('');
  L.push(`Gerado por SevenOptimizer v${p.meta.appVersion} em ${fmtDate(new Date().toISOString())}`);
  return L.join('\r\n');
}
...
<title>SevenOptimizer — Relatório</title><style>
...
<p class="dim">Gerado por SevenOptimizer v${esc(p.meta.appVersion)} — ${fmtDate(new Date().toISOString())}</p>
</body></html>`;
}
